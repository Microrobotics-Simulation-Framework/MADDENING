...
5. **`derivatives()` is the collision-free right-hand side.** It reads `gravity` from injected `params` by the same `{**self.params, **params}` rule as `update()` and adds it in the velocity's dtype (MADD-ANO-018 and the `BallNode` instance of MADD-ANO-013, both resolved in 0.4.0), but it never reads `table_position` or `elasticity`. `integrate_node` on this node therefore passes through the surface: from 0.001 above the table at -1 m/s, one 0.01 s Euler step lands at -0.009, where `update()` bounces to 0.0. Use `update()` for anything that reaches the table.
