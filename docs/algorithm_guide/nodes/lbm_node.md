...
3. streaming, periodic on every axis (`jnp.roll`), or across a halo in the sharded path, whose halos at the edges of the global grid are filled periodically so that it is the same step (`halo_boundary()`, below);
...
| Periodic halo fill at the global edges (the sharded step is `update`) | `maddening.nodes.lbm.LBMNode.halo_boundary` | `ShardedStencilNode`'s default `boundary`; a different fill is refused |
