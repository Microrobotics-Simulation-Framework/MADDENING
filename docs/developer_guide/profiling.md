...
| `coupling_overhead_ms` (`"measured"`) | the graph is recompiled with every group capped at one iteration and timed; the difference to the real step is what the extra iterations cost, and `coupling_per_iteration_ms` divides it by the mean number of extra iterations.  The graph, its state and compiled step are restored afterwards.  `measure_coupling=False` falls back to the old estimate (step minus the sum of isolated node times).  The two windows are timed at different points of the trajectory -- `mean_step_ms` just after a reset, the capped steps from wherever the run and the statistics pass left the state -- which is sound because a cap of one returns after a single staggered pass with no data-dependent control flow, so the capped step costs the same from any state (measured on the compute-bound `expensive-pair` fixture: pinning both windows moves the number by less than one standard error of the difference -- 0.03 SE over 10 repeats on a quiet box, 0.62 SE over 16 on a loaded one).  **Reported signed**: on a graph whose step is dominated by dispatch the difference is smaller than its own scatter and falls either side of zero (measured: negative in 11 of 30 runs of a two-spring pair), so a negative value is the honest report of an overhead the run could not resolve.  It used to be clamped at zero, which biased the mean upward by discarding only the negative half of the noise |
| `coupling_overhead_se_ms` | the standard error of that difference, propagated from the two windows' own per-step scatter -- no chosen threshold.  Below it the overhead was certainly not resolved and `str(report)` says `below resolution`; above it, not necessarily, because it captures the scatter *within* each window and not drift between two windows timed minutes apart.  On a shared machine the spread over repeated runs is the better guide.  `"measured"` only |
