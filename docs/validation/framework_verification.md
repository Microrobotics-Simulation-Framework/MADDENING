...
Nothing on this page is maintained by hand.  The tables are generated by
`scripts/generate_soup_tables.py` from the tree and from
`maddening.compliance.get_benchmark_registry()`, and
...
one of four registered benchmarks and seven of twelve test packages.
