...
*18 anomalies registered.  11 have a defect reachable in this version — every entry whose `resolution_status` is not `resolved` or `duplicate`, which is 8 `open` plus 3 `partially_resolved` whose residual risk is still live.  Rationale, workaround, affected components and verification evidence for each: `known_anomalies.yaml`.*
