...
#: Recorded evidence for the two band half-widths below.
#:
#: Every figure either constant's docstring quotes comes from these
#: four logs, which are in the tree and regenerated by the scripts
#: beside them.  An earlier revision of both docstrings quoted figures
#: that no ladder in the tree produces (5.02, 1.982, 1.847); they are
#: gone.  If you change a constant, re-run the fixture and quote the
#: new number — and note that
#: ``tests/verification/test_mms_order.py::TestTheOrderBandIsJustifiedByTheseNumbers``
#: pins the figures here against ``check_order``'s actual decisions, so
#: a number that drifts out of the band it is cited as being inside
#: fails the suite rather than sitting on the page.
_ORDER_BAND_FIXTURES = (
    "benchmarks/results/audit_040_r2/numerics/r8_order_band.log",
    "benchmarks/results/audit_040_r2/numerics/r1_heat_order_independent.log",
    "benchmarks/results/audit_040_r2/numerics/r6_declared_orders.log",
    "benchmarks/results/audit_040_r2/numerics/r10_lbm_order.log",
)

#: How far below its declared order a node's measurement may fall
#: before :func:`check_order` fails it.  For a declared 2 the lower
#: edge is ``2 - 0.25 = 1.75``; for a declared 4 it is ``3.75``.
#:
#: Measured, not chosen by intuition.  What is gated is the *finest*
...
#:
#: The defects this harness has found each fall a **full order or
#: more** short: MADD-ANO-007 measured 1.001 against a declared 2 and
#: MADD-ANO-008 measured 0.954 against a declared 4.  0.25 therefore
#: sits about 6x above the largest shortfall it must admit and 4x below
#: the smallest one it must reject, which is the only property being
#: claimed for it.
DEFAULT_ORDER_SHORTFALL = 0.25

#: How far *above* its declared order a measurement may go before
#: :func:`check_order` fails it.  For a declared 4 the upper edge is
#: ``4 + 1.0 = 5.00``; for a declared 2 it is ``3.00``.
#:
...
#: vanish identically and measures nothing.  In that case the error
#: reaches zero and :func:`check_order`'s ``isfinite`` guard usually
#: fires first; this band is the backstop for the near-exact case where
#: it does not.
#:
#: 1.0 is wide enough for every correct study recorded in
#: :data:`_ORDER_BAND_FIXTURES`.  Over four manufactured solutions on a
#: 10/20/40/80/160 ladder with the correct cubic ghost closure the
#: largest *pairwise* order anywhere is **4.126** (``tanh_bump``) and
#: the largest *finest-pair* order — the one actually gated — is
#: **4.001**.  Both are comfortably below the 5.00 edge.
#:
#: **What this band does not do, and why tightening it is a trap.**  It
#: is tempting to read it as the guard that catches a boundary closure
#: measuring too high.  It is not.  With the pre-0.4.0 flat-ended
#: profile, a linear ghost closure — genuinely only 2nd order —
#: measures **4.080** over the finest pair and passes.
#:
#: An edge of 4.05 would reject that one ladder.  But this constant is
#: global: it judges whatever ladder a study author writes, and
#: :func:`check_order` reads the *finest pair of the ladder it is
#: given*.  The correct cubic closure on ``tanh_bump`` measures
#: ``4.126, 4.025, 4.005, 4.001`` over 10/20/40/80/160, so a two- or
#: three-level study of it — which several studies in this tree are —
#: is gated on **4.126**.  Since ``4.080 < 4.126``, no global edge both
#: rejects the defect and admits that study.  Nor does re-deriving the
#: edge from the release's own two profiles help: both top out at
#: 3.957, and an edge fitted to them (about 3.96) false-fires on
#: ``exp_sin`` (3.999) and ``tanh_bump`` (4.001-4.126), neither of
#: which is doing anything wrong.
#:
#: What catches that defect is the requirement that the manufactured
#: solution be curved at both rod ends, pinned by
#: ``tests/verification/test_mms_order.py::TestTheSteadyProfileCanSeeABrokenScheme``.
#: On a profile that satisfies it the same linear closure measures
#: 2.000-2.003 and is rejected by the *shortfall*, 1.75 clear of the
#: 3.75 lower edge.  This constant is a coarse sanity check on
#: the study, not a defence of the scheme.
